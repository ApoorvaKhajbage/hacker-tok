...
export default function Home() {
  ...
  const loadStories = async (currentPage: number) => {
    ...
    try {
      ...
      console.log(`Fetched ${newStories.length} stories in ${Date.now() - start}ms`);
      ...
      toast({ title: "Error fetching stories", description: (error as Error).message });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <main className="min-h-screen bg-gray-100 dark:bg-gray-900">
      ...
      {/* Saved Posts Modal */}
      <SavedPosts isOpen={savedPostsOpen} onClose={toggleSavedPosts} savedStories={savedStories}
